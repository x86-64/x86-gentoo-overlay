...
SRC_URI="
	...
	mirror://gentoo/${BIS_P}
	bcrypt? (
		https://github.com/wclarie/openldap-bcrypt/archive/master.zip -> openldap-bcrypt.zip
	)
"
...
IUSE_CONTRIB="${IUSE_CONTRIB} bcrypt"
...
multilib_src_compile() {
	...
	if ! use minimal && multilib_is_native_abi ; then
		...
		if use bcrypt ; then
			cp -r ${WORKDIR}/openldap-bcrypt-master ${S}/contrib/slapd-modules/passwd/bcrypt
			pushd "${S}/contrib/slapd-modules/passwd/bcrypt" &>/dev/null || die "pushd contrib/slapd-modules/passwd/bcrypt"
			einfo "Compiling contrib-module: bcrypt"
			emake \
				LDAP_BUILD="${BUILD_DIR}" \
				CC="${CC}" libexecdir="/usr/$(get_libdir)/openldap"
			popd &>/dev/null || die
